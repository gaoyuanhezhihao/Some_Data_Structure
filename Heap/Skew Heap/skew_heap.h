#ifndef SKEW_HEAP_H_INCLUDED
#define SKEW_HEAP_H_INCLUDED
...
bool print_node(ofstream & out_file, TreeNode<Comparable> * pNode)
{
    ...
    {
        out_file << '\"'<<pNode->Element<<'\"' << "->" <<'\"'<< pNode->LeftChild->Element<<'\"' << endl;
        print_node(out_file, pNode->LeftChild);
    }
    if(pNode->RightChild != nullptr)
    {
        out_file <<'\"'<< pNode->Element<<'\"' << "->" <<'\"'<< pNode->RightChild->Element<<'\"' << endl;
        print_node(out_file, pNode->RightChild);
    }
    return true;
}
#endif // SKEW_HEAP_H_INCLUDED
